...
| ----------- | ------- | ----------- |
| OCM_GO_MODE | (none)  | Preset bundle: `strict`, `dev`, or `validator`. Use `validator` for federation-validator defaults. Scan, rate-limit, and SSRF details live in [configuration.md](configuration.md). Mount repo `configs/validator.toml` via `CONFIG` for the shipped sample TOML. |

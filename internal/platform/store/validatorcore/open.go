...
// first Attach in this process: leftover active-row recovery, per-target
// ready-waiter promotion, the passive TTL sweeps, the stall sweep, the
// retention sweep, and the terminal prune, in that order. Error propagation
// matches the per-step conventions: recovery is best-effort, every later
// step fails the pass.
func (c *Core) firstStartupMaintenance(ctx context.Context) error {
	...
	// Empty host considers every target that currently has a ready waiter.
...
// Attach after the first in this process: per-target ready-waiter
// promotion, the passive TTL sweeps, the retention sweep, and the terminal
// prune. The one-shot recovery and stall passes already ran and must not
// rerun.

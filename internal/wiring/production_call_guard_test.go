...
func assertAllowlistReferencesFunction(
	t *testing.T,
	root string,
	allowlistRel string,
	spec productionCallSpec,
	...
	t.Helper()
	if !allowlistReferencesFunction(t, root, allowlistRel, spec, false) {
		t.Fatalf("%s must reference %s; wiring.Build owns the seam", allowlistRel, spec.funcName)
	}
}

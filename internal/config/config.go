...
type Config struct {
	...
	ExternalOrigin string `json:"external_origin" toml:"external_origin"`
	...
	ExternalBasePath string `json:"external_base_path" toml:"external_base_path"`
	...
	OutboundHTTP OutboundHTTPConfig `json:"outbound_http" toml:"outbound_http"`
...
type ServerConfig struct {
	...
	BootstrapAdmin BootstrapAdminConfig `json:"bootstrap_admin" toml:"bootstrap_admin"`
}
...
type SignatureConfig struct {
	...
	OnDiscoveryError string `json:"on_discovery_error" toml:"on_discovery_error"`

	// AllowMismatch allows declared peer vs keyId host mismatch (dev-only)
	AllowMismatch bool `json:"allow_mismatch" toml:"allow_mismatch"`
}
...
type TLSConfig struct {
	...
	SelfSignedDir string `json:"self_signed_dir" toml:"self_signed_dir"`

	// ACME configuration
	ACME ACMEConfig `json:"acme" toml:"acme"`
}
...
type OutboundHTTPConfig struct {
	...
	ConnectTimeoutMS int `json:"connect_timeout_ms" toml:"connect_timeout_ms"`

	// MaxRedirects is the maximum number of redirects to follow
	MaxRedirects int `json:"max_redirects" toml:"max_redirects"`

	// MaxResponseBytes is the maximum response body size
	MaxResponseBytes int64 `json:"max_response_bytes" toml:"max_response_bytes"`
...
// Note: This returns dev-like defaults for backward compatibility.
// For production, use StrictConfig() or load via config.Load().
	...
	return &Config{
		...
		Server: ServerConfig{
			...
			BootstrapAdmin: BootstrapAdminConfig{
				Username: "admin",
				Password: "", // empty means auto-generate on first boot
			},
		},
		TLS: TLSConfig{
			Mode:          "selfsigned",
